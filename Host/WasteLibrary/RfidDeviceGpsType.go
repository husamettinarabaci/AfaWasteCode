...
func (res *RfidDeviceGpsType) GetByRedis(dbIndex string) ResultType {
	...
	resultVal = GetRedisForStoreApi(dbIndex, REDIS_RFID_GPS_DEVICES, res.ToIdString())
...
func (res *RfidDeviceGpsType) SelectSQL() string {
	return fmt.Sprintf(`SELECT Latitude,Longitude,Speed,GpsTime
	 FROM public.`+DATATYPE_RFID_GPS_DEVICE+` 
	 WHERE DeviceId=%f ;`, res.DeviceId)
}

//InsertSQL
func (res *RfidDeviceGpsType) InsertSQL() string {
	return fmt.Sprintf(`INSERT INTO public.`+DATATYPE_RFID_GPS_DEVICE+`  (DeviceId,Latitude,Longitude,Speed,GpsTime) 
	  VALUES (%f,%f,%f,%f,'%s') 
...
func (res *RfidDeviceGpsType) UpdateSQL() string {
	return fmt.Sprintf(`UPDATE public.`+DATATYPE_RFID_GPS_DEVICE+`  
